...
used in the current iteration of this analytical model to be presented here.\footnote{\hoss{Random variables (RV) are denoted as capitalized letters $X$, while constants are depicted with small letters $c$.}} 
...
\hoss{The command rate is denoted as $c$.}
...
spend some processing time $P$ in order to do so. \hoss{The processing time is a random variable which is assumed to follow a truncated normal distribution in our simulations.} Once finished, an update 
...
Fig.~\ref{fig:queuing-model} gives a graphical representation of the model for the online video game case. \hoss{Explanation of Figure~\ref{fig:queuing-model} is missing. What is the meaning of the other queues? Mit der Tabelle~\ref{tab:notation} und Update von Fig.~\ref{fig:queuing-model} sollte das klarer werden.} Moreover, Fig.~\ref{fig:tickrate-timeseries} shows a time series attempting to exemplify this for the simplified case of an online video game.

\begin{figure*}[!t]
	\centering
	\includegraphics[width=1.0\textwidth]{../models/e2e-lag-model.pdf}
	\caption{Abstract end-to-end lag queuing model representation in the online video game case. \hoss{Die Queues sind nicht ganz korrekt so. Es werden periodisch alle Events verarbeitet und weitergereicht. Hier sieht es nach FIFO und Einzelabarbeitung aus. Ausserdem sind die Variablen $N_C$ usw. nicht erklaert. Ausserdem wird im Model nicht erklaert, dass es einen Gaming Processing Delay am Game Server gibt.}}
...
\begin{figure}[!t]
	...
	\caption{Exemplary flow of events in online client-server game, and resulting end-to-end lag. \hoss{Notation aus Tabelle ~\ref{tab:notation} waere gut im Bild. }} %Delay values are given for a framerate of \SI{60}{\hertz} and a server tickrate of \SI{30}{\hertz}, the network latency will only show minor variations.}
...
\hoss{The model can be extended to take those delay factors into account. For the sake of simplicity, we have ignored them in this paper.}
