...
\midrule
$T$ & end-to-end lag (RV) & key performance measure \\
$D$ & network delay (RV) between game client and server & $D \sim LogN(\mu;\sigma)$ with $ \mu=\unit[50]{ms}; \sigma=\unit[50]{ms}$\\
$U$ & (inter arrival) time between user inputs (RV) & $U \sim Exp(\lambda)$; $\lambda=\unit[50]{ms}$\\
$b$ & input events are gathered and sent in time intervals $b$ (const.) & $b=\unit[]{}$ \\
$c$ & command rate $c=\nicefrac{1}{b}$ (const.) & $c=\unit[]{}$ \\
$t$ & game tick rate (const.) & $t=\unit[]{}$ \\
$f$ & frame rate (const.) & $f=\unit[]{}$ \\
\bottomrule
\end{tabu}
\end{table}
...
\begin{figure*}[!t]
	...
	\caption{Abstract end-to-end lag queuing model representation in the online video game case. \hoss{Die Queues sind nicht ganz korrekt so. Es werden periodisch alle Events verarbeitet und weitergereicht. Hier sieht es nach FIFO und Einzelabarbeitung aus. Ausserdem sind die Variablen $N_C$ usw. nicht erklaert. }}
