...
\subsection{Game Flavors}
%TODO: needs a better word than types, more like architectures. Feedback welcome


 The components of a locally 
running \textit{singleplayer game} and their interactions are 
depicted in Fig.~\ref{fig:component-model-local}.
...
A third typical component of online video games is the 
\textbf{command message rate}, i.e., the rate at which input is 
sent from the client to the server. as well as game state updates 
from the server to the clients do not need to be the same rate as 
