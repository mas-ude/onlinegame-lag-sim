...
\hoss{Wir haben doch eigentlich ein nebenlaeufiges System. Das wird hier nicht klar, sondern erweckt den falschen Eindruck. Eventuell andere Darstellung (siehe Scan). Formal koennte man das mit UML oder Petri-Netzen machen, ist aber nicht verstaendlich. Daher semi-formal graphical representation.}}
\label{alg:gameloop1}
\includegraphics[width=\columnwidth]{figure1videoGameLoop.jpg}
\end{figure}
...
\begin{table}[!t]
\caption{Exemplary table of some competitive and popular video game tickrates that are either known, speculated upon, or derived by counting update and command messages. Data was collected from various reliable and unreliable sources and should be taken as-is. 
...
architecture, and thus form an \textit{online video game} (see 
Fig.~\ref{fig:component-model-online}). 
...
\begin{figure}[!t]
  ...
  \caption{Components and interactions of a single-player video game.}
...
Finally, the only method to fully capture the end-to-end lag is to simultaneously record both the screen and input device through an external camera. 
