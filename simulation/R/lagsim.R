...
for(i in 1:par.userinput.events){
  ...
  arrivalAtServer <- nextTickR + D.up[i] # uplink
  
  nextTickS <- ceil((arrivalAtServer - tickrate.timedelta.0) / tickrate.timedelta) * tickrate.timedelta + tickrate.timedelta.0 # processing input at next game tick
  W[i,2] <- nextTickS - arrivalAtServer # (ms)
  
  arrivalAtClient <- nextTickS + B[i] + D.down[i]
